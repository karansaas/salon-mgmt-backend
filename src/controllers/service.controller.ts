...
const parseService = (body: unknown, required: boolean): ServiceInput | Partial<ServiceInput> => {
  ...
  for (const field of ['duration', 'price'] as const) { if (required && input[field] === undefined) throw new AppError(400, `${field} is required`); if (input[field] !== undefined) { const value = Number(input[field]); if (!Number.isFinite(value) || value <= 0) throw new AppError(400, `${field} must be greater than zero`); output[field] = value; } }
