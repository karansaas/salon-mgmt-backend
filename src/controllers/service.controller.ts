...
import { ServiceInput, createService, deactivateService, getService, getServicePerformance, listServices, toServiceResponse, updateService } from '../services/service.service.js';
...
export const getServicePerformanceById = async (req: Request, res: Response, next: NextFunction): Promise<void> => { try { res.json({ performance: await getServicePerformance(routeId(req.params.id)) }); } catch (error) { next(error); } };
